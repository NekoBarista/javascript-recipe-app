...
const handleRandomClick = () => {
   ...
   newarray.forEach((ingredient)=>{
    ...
    ).then(function (response)
    {
        ...
        recipe.Body = response.data.recipes[0].instructions.replaceAll('<li>', ' ').replaceAll('<ol>' ,' ').replaceAll('<b>',' ').replaceAll('<ul>', ' ').replaceAll('<st>',' ').replaceAll('</st>',' ').replaceAll('</b>',' ').replaceAll('</ol>', ' ').replaceAll('</li>', ' ').replaceAll('</ul>', ' ').replaceAll('<br>', ' ').replaceAll('</br>', ' ').trim()
